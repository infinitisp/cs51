...
let rec list_to_stream (lst : obj list) : event stream =
  let rec list_to_stream_rec nlst rest : event stream =
    match nlst with
    |[] -> shift_start rest (list_to_stream lst)
    |x::y -> (match x with
	     |Note (p,t,v) -> (fun () -> Cons (Tone (rest, p, v), (fun () ->
					       Cons (Stop (t, p), 
					       list_to_stream_rec y 0.))))
	     |Rest t -> list_to_stream_rec y t
	     )
...
let rec pair (a : event stream) (b : event stream) : event stream =
  match (a (), b ()) with
  |(Cons(e1, astr), Cons(e2,bstr)) -> 
    let diff = (time_of_event e1) -. (time_of_event e2) in
    if diff <= 0. then
      (fun () -> Cons(e1, pair astr 
			       (shift_start ((-1.) *. (time_of_event e1)) b)))
    else
      (fun () -> Cons(e2, pair 
			    (shift_start ((-1.) *. (time_of_event e2)) a) bstr))
...
let rec transpose (str : event stream) (half_steps : int) : event stream =
    match str () with
    |Cons (Tone (x,y,z), a) -> 
      (fun () -> Cons (Tone(x, transpose_pitch y half_steps,z),
		       transpose a half_steps))
    |Cons (Stop (x, y), a) ->
      (fun () -> Cons (Stop(x, transpose_pitch y half_steps),
		       transpose a half_steps))
...
<<<<<<< HEAD
let pitch_to_string ((p, i) : pitch) =
  let s = match p with | A -> "A" | Bb -> "Bb" | B -> "B"| C -> "C" | Db -> "Db"
		  | D -> "D" | Eb -> "Eb" | E -> "E" | F -> "F" | Gb -> "Gb" 
		  | G -> "G" | Ab -> "Ab"
  in
  "(" ^ s ^ ", " ^ (string_of_int i) ^ ")"

let rec printstream e n =
  if n = 0 then () else
  match e () with
  |Cons (e, next) ->
    match e with 
    |Tone (x, y, z) -> Printf.printf "%s" ("Tone of (" ^ (Float.to_string x) ^ 
					   ", " ^ (pitch_to_string y) ^ ", " ^
					   (string_of_int z) ^ ")\n");
		       printstream next (n-1)
    |Stop (x, y) -> Printf.printf "%s" ("Stop of (" ^ (Float.to_string x) ^
					  ", " ^ (pitch_to_string y) ^ ")\n");
		       printstream next (n-1)
...
let canon = pair (pair (pair bass (shift_start 2. melody)) (shift_start 4. melody))
		 (shift_start 6. melody);;
...
let minutes_spent : int = 180 (* TOO many... your explanation of how objs 
			      * translate to events was very difficult 
			      * Once I got that, thanks to a student's post
			      * on piazza the rest was really easy *)
