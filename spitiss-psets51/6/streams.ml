...
let rec doubletree () =
  Stem (1, mapt (( * ) 2) doubletree, mapt (( * ) 2) doubletree)

let rec treenats () =
  Stem (1, zipt (+) doubletree treenats, zipt (+) (zipt (+) doubletree doubletree) treenats)

(* NOTE: I'm not SUPER confident as to why this works... it sort of makes sense,
 * and that's why I was able to get it right, but I couldn't teach it. It 
 * would be very helpful to go into this recursion in more depth. Why is the 
 * treenats changing ... or even the doubletree changing upon recursion... i.e., 
 * why isn't double tree just 1 , 2 , 2 , 2 , 2... etc. *)

assert (headt (rtail treenats) = 3)
assert (headt (ltail (ltail treenats)) = 4)
assert (headt (ltail (ltail (ltail treenats))) = 8)
assert (headt (rtail (ltail (ltail treenats))) = 9)
assert (headt (ltail (rtail (ltail treenats))) = 10)
assert (headt (rtail (rtail (ltail treenats))) = 11)
assert (headt (ltail (ltail (rtail treenats))) = 12)
;;
...
let rec nth (n:int) (s:'a stream) : 'a =
  if n <= 0 then head s else
  match s with
  |Cons (_,t) -> nth (n-1) (Lazy.force t)
;;
...
let rec merge (s1:int stream) (s2:int stream) : int stream =
  match Int.compare (head s1) (head s2) with
  | 0   -> merge s1 (tail s2)
  |(-1) -> Cons (head s1, lazy (merge (tail s1) s2))
  | _   -> Cons (head s2, lazy (merge s1 (tail s2)))
;;
...
(*
 *  Answer: because streams are infinite, we can enter an infinite loop if
 *  equality keeps getting called (since it's not a lazy call)
 *  This won't blow the stack because its a tail call, but it will loop
 *  forevs. 
 *)
...
let scale n = map (( * ) n) ;; (* is this a trick question? *)
...
let rec selectivestream = Cons(1, lazy(merge (scale 3 selectivestream) 
					     (scale 5 selectivestream))) ;;
...
let minutes_spent : int = 90 (* this was a good assignment, unlike
			      * certain previous ones *)
